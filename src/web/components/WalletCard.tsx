...
const WalletCard: NextPage<WalletCardProps> = ({ detail, type }) => {
  ...
  async function update() {
    ...
    if (detail.identifier === 'phone') {
      tx = await phoneLinkContract.editProfile(
        detail.name,
        value,
  ...
  async function makePrimary() {
    ...
    const tx = await phoneLinkContract.makePrimary(
                  ...
                  {Boolean(otp) === false ? (
                        ...
                        <div>
                          {savebutton === true ? (
                            <div id="sign-in-button">
                              ...
                              <button
                                className={`ml-2 text-[#fc0303] hover:text-[#fc0303]`}
                                onClick={() => {
                                  setSaveButton(false)
                                  setValue('')
                                }}
                            ...
                            <div
                              className={`${style.collectionName} cursor-pointer hover:text-[#03ecfc]`}
                    ...
                    <div className={`${style.searchBarVerify} mt-2`}>
                      ...
                      >
                        ...
                        />
                        <button className={`${style.button} ml-2 p-1`}>
                          Verify
                        </button>
                      </form>
                    </div>
              ...
              <div className={style.details}>
                <div className={style.info}>
                  <div className={style.infoLeft}>
                    ...
                    <div className={style.collectionName}>
                      {detail.typeOfIdentifier}: {detail.identifier}
                    </div>
                    <div className={style.collectionName}>
                      Wallet: {detail.connectedWalletAddress}
                    </div>
                    <div className={style.collectionName}>
                  ...
                  <div className={style.infoRight}>
                    <div>
                      {detail.isPrimaryWallet ? null : (
                        <div
                          className={`${style.collectionName} cursor-pointer text-[#f54b42] hover:text-[#03ecfc]`}
                          onClick={() => makePrimary()}
                        >
                          <u>Set Primary</u>
                        </div>
